...
// 3. 当前组件主要依赖：{ onMounted, reactive, ref } <- vue；{ useRoute, useRouter } <- vue-router；{ ElMessage } <- element-plus；DetailPageTemplate <- src/components/common/business/DetailPageTemplate.vue；{ getUserDetailApi } <- src/api/system.js
