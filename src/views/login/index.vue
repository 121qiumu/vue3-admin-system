...
<script setup>
// 学习注释：
// 1. 文件角色：这是登录页，负责账号登录表单、账号示例、登录流程和成功跳转。
// 2. 所在分层：页面层：真正承载业务展示和用户操作的界面。
// 3. 当前组件主要依赖：{ computed, onMounted, reactive, ref } <- vue；{ useI18n } <- vue-i18n；{ useRoute, useRouter } <- vue-router；{ getLoginAccountListApi } <- src/api/auth.js；{ getButtonPermissionApi, getMenuListApi } <- src/api/permission.js；{ useLocale } <- src/hooks/useLocale.js；其余依赖 6 项
